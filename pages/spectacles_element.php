...
<html lang="fr">
	...
	<body>
        ...
        $a = $db->prepare("SELECT nom, artiste, type, photo FROM spectacles WHERE id = :id");
        ...
        $b = $db->prepare("SELECT note, commentaire FROM spectacles_users WHERE id_user = :id_user AND id_spectacles = :id_spectacles ");
        $b->execute([ 'id_user' => $id_user_active, 'id_spectacles' => $id_element ]);
        $resultb = $b->fetch();
        ...
        if(isset($_POST["modifelement"])){
            ...
            $m = $db->prepare("UPDATE spectacles_users SET note = ?, commentaire = ?, date_realise = ? WHERE id_user = ? AND id_spectacles = ?");
            $m->execute([$note, $commentaire, date("Y-m-d h:i:s",time()), $id_user_active, $id_element]);

            header("Location: spectacles_element.php?id=".$id_element."");
            die();
        }

        if(isset($_POST["supp_element"])){

            $n = $db->prepare("DELETE FROM spectacles_users WHERE id_user = :id_user AND id_spectacles = :id_spectacles");
            $n->execute(['id_user' => $id_user_active, 'id_spectacles' => $id_element]);

            header("Location: spectacles_realise.php");
            die();
        }
        ...
        <main class="page_main">
            ...
            <div id="retour_reglage">
                <a href="spectacles_realise.php" class="retour"><img src="../assets/img/retour.svg" alt="Retour" width="25px"> Retour</a>
                ...
                <div id="myModalb" class="modal">
                    <div class="modal-content modalb">
                        ...
                        <div id="contenu_reglage">
                            <h3 style="font-size:25px; margin-bottom:3rem;" class="hparam">RÉGLAGE DU SPECTACLE</h3>
                            ...
                            <div id="supprime_element" style="display:none;">
                                <form method="post" id="sup_element">
                                    <label style="display:inline-block; height:60px">VOULEZ-VOUS VRAIMENT<br>SUPPRIMER CE SPECTACLE?</label><br>
            ...
            <div id="contenant_element">
                ...
                <div id="infos_element">
                    ...
                    <div id="infos">
                        ...
                        <h3 class="sous-t"><?php echo $artiste ?></h3>
                        <p><?php echo $type?></p>
                    </div>
                </div>
