...
<html lang="fr">
	...
	<body>
        ...
        $a = $db->prepare("SELECT nom, preparation, ingredient, photo FROM gastronomie WHERE id = :id");
        $a->execute([ 'id' => $id_element]);
        $result = $a->fetch();
        
        $nom= $result[0];
        $preparation= $result[1];
        $ingredient= $result[2];
        $photo= $result[3];

        $b = $db->prepare("SELECT note, commentaire FROM gastronomie_users WHERE id_user = :id_user AND id_gastronomie = :id_gastronomie ");
        $b->execute([ 'id_user' => $id_user_active, 'id_gastronomie' => $id_element ]);
        $resultb = $b->fetch();
        ...
        if(isset($_POST["modifelement"])){
            ...
            $m = $db->prepare("UPDATE gastronomie_users SET note = ?, commentaire = ?, date_realise = ? WHERE id_user = ? AND id_gastronomie = ?");
            $m->execute([$note, $commentaire, date("Y-m-d h:i:s",time()), $id_user_active, $id_element]);

            header("Location: gastronomie_element.php?id=".$id_element."");
            die();
        }

        if(isset($_POST["supp_element"])){

            $n = $db->prepare("DELETE FROM gastronomie_users WHERE id_user = :id_user AND id_gastronomie = :id_gastronomie");
            $n->execute(['id_user' => $id_user_active, 'id_gastronomie' => $id_element]);

            header("Location: gastronomie_realise.php");
            die();
        }
        ...
        <main class="page_main">
            ...
            <div id="retour_reglage">
                <a href="gastronomie_realise.php" class="retour"><img src="../assets/img/retour.svg" alt="Retour" width="25px"> Retour</a>
                ...
                <div id="myModalb" class="modal">
                    <div class="modal-content modalb">
                        ...
                        <div id="contenu_reglage">
                            <h3 style="font-size:25px; margin-bottom:3rem;" class="hparam">RÉGLAGE DE LA RECETTE</h3>
                            ...
                            <div id="supprime_element" style="display:none;">
                                <form method="post" id="sup_element">
                                    <label style="display:inline-block; height:60px">VOULEZ-VOUS VRAIMENT<br>SUPPRIMER CETTE RECETTE?</label><br>
            ...
            <div id="contenant_element">
                ...
                <div id="infos_element">
                    ...
                    <div id="infos">
                        ...
                        <h3 class="sous-t"><?php echo $ingredient ?></h3>
