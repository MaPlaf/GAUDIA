	...
	<body>
        ...
        <?php 
            ...
            if(isset($_POST["add_element"])){
                ...
                $appel_script = '<BODY onLoad="displayFunction(), envoie_donnee(`nom_activite_pese`,`'.$nom.'`,`photo_activite_pese`,`'.$photo.'`);">';
                ...
                if($resultat_a == 0){
                    ...
                    if($resultat_d == 0){
                        ...
                        if($resultdd ='null'){
                            ...
                            $deja_existe = "<p style='color:red; text-align:center;'>Cette activité a déjà été fait!</p>";
            ...
            if(isset($_POST["ajout_realise"])){
                ...
                header("Location: activite_element.php?id=".$resultee['id']."");
                die();
            }
            ...
            function genere_liste($ordre, $colone){
                ...
                foreach($db->query("SELECT id_activite, note, commentaire, date_realise FROM activite_users WHERE id_user = $id_user_active AND date_realise != 'null'") as $row ){
                    $id = $row[0];
                    $note = $row[1];
                    $commentaire = $row[2];
                    $date_realise = $row[3];

                    foreach($db->query("SELECT nom, emplacement, description, photo FROM activite WHERE id = $id") as $row ){
                        $nom = $row[0];
                        $emplacement = $row[1];
                        $description = $row[2];
                        $photo = $row[3];

                        array_push($tableau, array('id'=>$id, 'note'=>$note, 'commentaire'=>$commentaire, 'date_realise'=>$date_realise, 'nom'=>$nom, 'emplacement'=>$emplacement, 'description'=>$description, 'photo'=>$photo,  ));
                    }
                }
                ...
                if(isset($_POST["filtre_ajout"])){
                    ...
                    if($emplacement != ""){
                        for ($row = 0; $row < count($tableau); $row++) {
                            if(stripos($tableau[$row]['emplacement'], $emplacement) !== false){
                                array_push($emplacement_tableau, $tableau[$row]);
                                
                            }
                        }
                        array_push($filtres_appliques, $emplacement_tableau);
                    }
                    ...
                    for ($row = 0; $row < count($result); $row++) {
                        ...
                        $listes_listes = $listes_listes . 
                            '<div class="liste_element" onclick="redirige(`activite_element.php?id='.$result[$row]['id'].'`);">
                    ...
                    for ($row = 0; $row < count($tableau); $row++) {
                        ...
                        $listes_listes = $listes_listes . 
                            '<div class="liste_element" onclick="redirige(`activite_element.php?id='.$tableau[$row]['id'].'`);">
            ...
            $f = $db->prepare("SELECT id_activite, date_realise FROM activite_users WHERE id_user = $id_user_active AND date_realise != 'null'");
            $f->execute();
            $nb_res = $f->rowCount();
            ...
            if(($nb_res !== 0)){
                ...
                $vide = "<h3 style='margin-top:5rem;'>Vous n'avez pas encore marqué d'activite comme réalisée!</h3>";
            }
        ?>
                    ...
                    <form method="post" id="modif_filtre" class="form_para">
                        ...
                        <div>
                            ...
                            <div>
                                ...
                                <input type="text" name="emplacement" id="emplacement" class="cInput">
                            </div>
                            <div>
