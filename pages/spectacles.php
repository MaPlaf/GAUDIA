	...
	<body>
        ...
        <?php
            ...
            function genere_liste($ordre){
                ...
                foreach($db->query("SELECT * FROM spectacles_listes WHERE id_user = $id_user_active ORDER BY $ordre") as $row){
                    $listes_listes = $listes_listes . 
                                '<div class="liste_listes">
                                    <a href="spectacles_liste.php?id='.$row[0].'&liste='.$row[2].'"> '.$row[2] .'</a>
                                </div>';
                }
            }

            $e = $db->prepare("SELECT * FROM spectacles_listes WHERE id_user = $id_user_active");
            $e->execute();
            $nb_res = $e->rowCount();
            ...
            if(isset($_POST["add_list"])){
                ...
                if(!empty($nom_liste)){
                    ...
                    $c = $db->prepare("SELECT nom FROM spectacles_listes WHERE nom = :nom AND id_user = :id_user");
                    ...
                    if($result == 0){
                        ...
                        $q = $db->prepare("INSERT INTO spectacles_listes(id_user, nom) VALUES(:id_user, :nom)");
                        $q->execute([
                            'id_user' => $id_user_active,
                            'nom' => $nom_liste,
                            ]);
                        
                        $qq = $db->prepare("SELECT id FROM spectacles_listes WHERE nom = :nom AND id_user = :id_user");
                        $qq->execute([ 'nom' => $nom_liste, 'id_user' => $id_user_active]);
                        $result_qq = $qq->fetch();

                        header("Location: spectacles_liste.php?id=".$result_qq['id']."&liste=".$nom_liste."");
                        die();
