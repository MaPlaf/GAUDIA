...
<html lang="fr">
	...
	<body>
        ...
        $a = $db->prepare("SELECT ville, pays, photo, description FROM voyage WHERE id = :id");
        ...
        $b = $db->prepare("SELECT note, commentaire FROM voyage_users WHERE id_user = :id_user AND id_voyage = :id_voyage ");
        $b->execute([ 'id_user' => $id_user_active, 'id_voyage' => $id_element ]);
        $resultb = $b->fetch();
        ...
        if(isset($_POST["modifelement"])){
            ...
            $m = $db->prepare("UPDATE voyage_users SET note = ?, commentaire = ?, date_realise = ? WHERE id_user = ? AND id_voyage = ?");
            $m->execute([$note, $commentaire, date("Y-m-d h:i:s",time()), $id_user_active, $id_element]);

            header("Location: voyage_element.php?id=".$id_element."");
            die();
        }

        if(isset($_POST["supp_element"])){

            $n = $db->prepare("DELETE FROM voyage_users WHERE id_user = :id_user AND id_voyage = :id_voyage");
            $n->execute(['id_user' => $id_user_active, 'id_voyage' => $id_element]);

            header("Location: voyage_realise.php");
            die();
        }
        ...
        <main class="page_main">
            ...
            <div id="retour_reglage">
                <a href="voyage_realise.php" class="retour"><img src="../assets/img/retour.svg" alt="Retour" width="25px"> Retour</a>
                ...
                <div id="myModalb" class="modal">
                    <div class="modal-content modalb">
                        ...
                        <div id="contenu_reglage">
                            <h3 style="font-size:25px; margin-bottom:3rem;" class="hparam">RÉGLAGE DU VOYAGE</h3>
                            ...
                            <div id="supprime_element" style="display:none;">
                                <form method="post" id="sup_element">
                                    <label style="display:inline-block; height:60px">VOULEZ-VOUS VRAIMENT<br>SUPPRIMER CE VOYAGE?</label><br>
            ...
            <div id="contenant_element">
                ...
                <div id="infos_element">
                    <img src="<?php echo $photo?>" alt="<?php echo $ville?>">

                    <div id="infos">
                        <h1><?php echo mb_strtoupper($ville)?></h1>
                        <h3 class="sous-t"><?php echo $pays ?></h3>
