	...
	<body>
        ...
        <?php 
            ...
            if(isset($_POST["add_element"])){
                ...
                if($resultat_a == 0){
                    ...
                    $c = $db->prepare("SELECT id FROM spectacles_users WHERE id_user = :id_user AND id_spectacles = :id_spectacles");
                    $c->execute([ 'id_user' => $id_user_active, 'id_spectacles' => $resultb['id']]);
                    $resultc = $c->fetch();

                    $s = $db->prepare("INSERT INTO spectacles_elements_listes(id_liste, id_spectacles_user) VALUES(:id_liste, :id_spectacles_user)");
                    $s->execute([
                        'id_liste' => $id_liste,
                        'id_spectacles_user' => $resultc['id']
                        ]);

                    header("Location: spectacles_liste.php?id=".$id_liste."&liste=".$nom_liste."");
                    die();
                    ...
                    if($resultat_d == 0){
                        ...
                        $j = $db->prepare("SELECT id FROM spectacles_users WHERE id_user = :id_user AND id_spectacles = :id_spectacles");
                        $j->execute([ 'id_user' => $id_user_active, 'id_spectacles' => $resulta['id']]);
                        $resultj = $j->fetch();

                        $k = $db->prepare("INSERT INTO spectacles_elements_listes(id_liste, id_spectacles_user) VALUES(:id_liste, :id_spectacles_user)");
                        $k->execute([
                            'id_liste' => $id_liste,
                            'id_spectacles_user' => $resultj['id']
                            ]);
                        ...
                        if($resultdd ='null'){
                            ...
                            $e = $db->prepare("SELECT * FROM spectacles_elements_listes WHERE id_liste = :id_liste AND id_spectacles_user = :id_spectacles_user");
                            $e->execute(['id_liste' => $id_liste, 'id_spectacles_user' =>$resultd['id']]);
                            $resultat_e = $e->rowCount();
                            $resulte = $e->fetch();

                            if($resultat_e == 0){
                                $t = $db->prepare("INSERT INTO spectacles_elements_listes(id_liste, id_spectacles_user) VALUES(:id_liste, :id_spectacles_user)");
                                $t->execute([
                                    'id_liste' => $id_liste,
                                    'id_spectacles_user' => $resultd['id']
                                    ]);
                                ...
                                $deja_existe = "<p style='color:red; text-align:center;'>Ce spectacle est déjà dans la liste!</p>";
                            ...
                            $deja_existe = "<p style='color:red; text-align:center;'>Ce spectacle a déjà été vu!</p>";
                            echo '<script type="text/javascript">function displayFunction(){document.getElementById("myModal").style.display = "block";};</script>';
                            echo '<BODY onLoad="displayFunction()">';
                        }
                    }
            ...
            function genere_listedate($ordre){
                ...
                foreach($db->query("SELECT id_spectacles_user FROM spectacles_elements_listes WHERE id_liste = $id_liste ORDER BY $ordre") as $row){
                    foreach($db->query("SELECT id_spectacles FROM spectacles_users WHERE id = $row[0]") as $row){
                        foreach($db->query("SELECT nom, photo FROM spectacles WHERE id = $row[0]") as $row){
                            $listes_listes = $listes_listes . 
                                '<div class="liste_element">
                                    <img src="'.$row[1].'" alt="'.$row[0] .'">
                                    <h5>'.$row[0] .'</h5>
                                    <div class="survol">
                                        <button name="ajout_realise" onclick="ouvrir_modal(`myModalc`); envoie_donnee(`nom_spectacles_pese`,`'.$row[0].'`,`photo_spectacles_pese`,`'.$row[1].'`);" class="bouton_a btn_survol">MARQUER COMME RÉALISÉ</button>
                                        <form method="POST">
            ...
            function genere_listealpha($ordre){
                ...
                foreach($db->query("SELECT id_spectacles_user FROM spectacles_elements_listes WHERE id_liste = $id_liste") as $row){
                    foreach($db->query("SELECT id_spectacles FROM spectacles_users WHERE id = $row[0]") as $row){
                        foreach($db->query("SELECT nom, photo FROM spectacles WHERE id = $row[0]") as $row ){
                            $tableau[$row[0]] = $row[1];
                        }
                    ...
                    foreach ($tableau as $key => $val) {
                        $listes_listes = $listes_listes . 
                            '<div class="liste_element">
                                ...
                                <h5>'.$key .'</h5>
                                <div id="class">
                                        <button name="ajout_realise'.$val.'" id="'.$key .'" onclick="ouvrir_modal(`myModalc`); envoie_donnee(`nom_spectacles_pese`,`'.$val.'`,`photo_spectacles_pese`,`'.$key .'`);" class="bouton_a btn_survol">MARQUER COMME RÉALISÉ</button>
                                    <form method="POST">
            ...
            $f = $db->prepare("SELECT * FROM spectacles_elements_listes WHERE id_liste = $id_liste");
            $f->execute();
            $nb_res = $f->rowCount();
            ...
            if(($nb_res !== 0)){
                ...
                $vide = "<h3 class='vide_h3' style='margin-top:5rem;'>Vous n'avez pas encore ajouté de spectacles à cette liste!</h3>";
            }

            if(isset($_POST["modifnom"])){
                extract($_POST);

                $m = $db->prepare("UPDATE spectacles_listes SET nom = ? WHERE id = ?");
                $m->execute([$nom, $id_liste]);

                header("Location: spectacles_liste.php?id=".$id_liste."&liste=".$nom."");
                die();
            }

            if(isset($_POST["supp_liste"])){

                $n = $db->prepare("DELETE FROM spectacles_elements_listes WHERE id_liste = :id_liste");
                $n->execute(['id_liste' => $id_liste]);

                $o = $db->prepare("DELETE FROM spectacles_listes WHERE id = ?");
                $o->execute([$id_liste]);
            ...
            if(isset($_POST["supprime_element"])){
                ...
                $aa = $db->prepare("SELECT id FROM spectacles WHERE nom = :nom");
                $aa->execute(['nom' => $nom]);
                $resultaa = $aa->fetch();

                $bb = $db->prepare("SELECT id FROM spectacles_users WHERE id_user = :id_user AND id_spectacles = :id_spectacles");
                $bb->execute(['id_user' => $id_user_active, 'id_spectacles' =>$resultaa['id']]);
                $resultbb = $bb->fetch();

                $o = $db->prepare("DELETE FROM spectacles_elements_listes WHERE id_liste = ? AND id_spectacles_user = ?");
        ...
        <main class="page_main">
            ...
            <div id="retour_reglage">
                <a href="spectacles.php" class="retour"><img src="../assets/img/retour.svg" alt="Retour" width="25px"> Retour</a>
